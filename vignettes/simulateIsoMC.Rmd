...
rasterTemplate <- terra::mask(terra::crop(terra::rast(rasterTemplate), OVEN_single_poly),
                               OVEN_single_poly)
...
relativeAbund  <- relativeAbund/terra::global(relativeAbund, fun = "sum")
```
...
iso <- terra::crop(terra::rast(featherIso), terra::ext(c(-180,-40,25,85)))
isocut <- terra::classify(iso, rcl = seq(terra::global(iso, fun = "min", na.rm = TRUE)$min, terra::global(iso, fun = "max", na.rm = TRUE)$max,12))
targetRanges[[4]] <- terra::as.polygons(isocut)


# 12*2 isotope units
isocut <- terra::classify(iso, rcl = seq(terra::global(iso, fun = "min", na.rm = TRUE)$min, terra::global(iso,fun = "max",na.rm = TRUE)$max,24))
...
iso_dat <- data.frame(Site = rep(1:3,each = 100),
                      xcoords = c(sf::st_coordinates(Site1)[,1],sf::st_coordinates(Site2)[,1],sf::st_coordinates(Site3)[,1]),
                      ycoords = c(sf::st_coordinates(Site1)[,2],sf::st_coordinates(Site2)[,2],sf::st_coordinates(Site3)[,2]),
                      targetSite = unlist(unclass(sf::st_intersects(rbind(Site1,Site2,Site3),
                                        targetRanges[[3]]))),
                      featherIso = terra::extract(terra::rast(featherIso),terra::vect(rbind(Site1,Site2,Site3))))
...
originDist <- MigConnectivity::distFromPos(st_coordinates(sf::st_centroid(originSites,
                                                            byid = TRUE)))
targetDistMC <- MigConnectivity::distFromPos(sf::st_coordinates(sf::st_centroid(targetRanges[[3]], 
                                                              byid = TRUE)))
```
...
output.sims <- foreach(target=1:nTargetSetups,
                       .combine = rbind,
                       .export = ls(.GlobalEnv),
                       .packages = c("terra","sf","MigConnectivity")) %dopar% {
#for(target in 1:nTargetSetups){
  set.seed(9001)
  targetSites <- targetRanges[[target]]
  targetDist <- distFromPos(sf::st_coordinates(sf::st_centroid(targetSites, byid = TRUE)))
  # Extract simulated data
  iso_dat <- data.frame(Site = rep(1:3,each = 100),
                        xcoords = c(sf::st_coordinates(Site1)[,1],sf::st_coordinates(Site2)[,1],sf::st_coordinates(Site3)[,1]),
                        ycoords = c(sf::st_coordinates(Site1)[,2],sf::st_coordinates(Site2)[,2],sf::st_coordinates(Site3)[,2]),
  ...
  for (sim in 1:nSims) {
    ...
    targetPoints <- sf::st_as_sf(targetPoints0, crs = 4326)
  
    # Extract simulated data
    iso_dat <- data.frame(Site = originAssignment,
                          xcoords = targetPoints0[,1],
                          ycoords = targetPoints0[,2],
                          targetSite = unlist(unclass(sf::st_intersects(targetPoints,targetSites))),
                          featherIso = terra::extract(terra::rast(featherIso),terra::vect(targetPoints)))
    ...
    originPoints <- sf::st_as_sf(originCap)
    sf::st_set_crs(originPoints, 4326) 
