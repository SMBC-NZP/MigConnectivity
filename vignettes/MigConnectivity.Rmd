...
#### EXAMPLE 7
`estMC` Estimate strength of migratory connectivity incorporating uncertainty from limited sample size (no other sampling error).  Here we simulated data using the 13th scenario from example 4 above ("CentroidSampleBreeding4"; make sure you run example 4 code before running this section).  We changed the total sample size from 1000 to 100 animals and the total abundance from 50000 to 2500 animals.  
...
#### EXAMPLE 8
`estMC` Estimate strength of migratory connectivity incorporating uncertainty from location error associated with light-level geolocators.  Again, we simulated data using the 13th scenario from example 4 above ("CentroidSampleBreeding4"; make sure you run examples 4 and 7 code before running this section).  
...
# Make summary table and figures of results with (Example 8) and without 
# (Example 7) location error
...
write.csv(qualities.all, file = '../qualities.all.csv', row.names = F)
