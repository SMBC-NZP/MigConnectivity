...
# note - the link should be a single continuous link without breaks. 
# Breaks were added here so users could see the entire link 

winterBirds <- dget("https://raw.githubusercontent.com/
...
# note - the link below should be a single continuous link without breaks. 
# Breaks were added here so users could see the entire link 
...
LongError<-rep(NA,length(winterBirds)) # 16 birds recovered in non-breeding season
...
LongError[Jam] <- unlist(lapply(NB_GLmeters[Jam],
                         FUN = function(x){mean(x@coords[,1]-CapLocsM@coords[1,1])}))

LongError[Fla] <- unlist(lapply(NB_GLmeters[Fla],
                         FUN = function(x){mean(x@coords[,1]-CapLocsM@coords[2,1])}))

LongError[PR] <- unlist(lapply(NB_GLmeters[PR],
                        FUN = function(x){mean(x@coords[,1]-CapLocsM@coords[3,1])}))
...
LatError[Jam] <- unlist(lapply(NB_GLmeters[Jam],
                        FUN = function(x){mean(x@coords[,2]-CapLocsM@coords[1,2])}))

LatError[Fla] <- unlist(lapply(NB_GLmeters[Fla],
                        FUN = function(x){mean(x@coords[,2]-CapLocsM@coords[2,2])}))

LatError[PR] <- unlist(lapply(NB_GLmeters[PR],
                        FUN = function(x){mean(x@coords[,2]-CapLocsM@coords[3,2])}))
...
Lambert <- "+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 
            +ellps=GRS80 +datum=NAD83 +units=m +no_defs"

resampleProjection <- "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 
...
WinteringPolys <- lapply(winteringPos14,
                         toPoly,
                         projection.in = WGS84, 
                         projection.out = Lambert)

winteringPolys <- lapply(winteringPos14,
                         toPoly,
                         projection.in = WGS84,
                         projection.out = WGS84)

winteringPolys2 <- lapply(winteringPos14,
                          toPoly,
                          projection.in = WGS84,
                          projection.out = resampleProjection)
...
for (i in 13) { # only run one scenario
  ...
  animalLoc14[[i]] <- changeLocations(animalLoc14.0,
    ...
    target.point.sample <- sp::SpatialPoints(results14.2[[sim]]$targetPointSample,
                                             sp::CRS(Lambert))
