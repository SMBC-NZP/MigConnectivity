...
`estMC` Estimate the strength of migratory connectivity (MC) while incorporating location uncertainty   
...
relN
#equal absolute abundance among the three breeding regions, each must be >= 1 
N <- rep(3000, nBreeding)
...
```{r eval = FALSE}
install.packages(c('rgeos', 'shape', 'raster', 'maptools'))
```
...
### EXAMPLES 4-9 (from sampling regime simulations in Cohen et al. in revision)
...
compare15 <- data.frame(Scenario = c("True", 
                        ...
                        MCA = c(MC15, rep(NA, nScenarios15)), 
                        Mantel = c(MC15, rep(NA, nScenarios15)))

compare15.array <- array(NA, c(nSims15, nScenarios15, 3), dimnames = list(1:nSims15,
...
for (sim in 1:nSims15) {
  ...
  for (i in 1:nScenarios15) {
    ...
    compare15.array[sim, i, 'MCA'] <- calcMC(originDist  = breedDist15[[i]], 
                                             targetDist = nonbreedDist15[[i]], 
                                             originRelAbund = breedingN15[[i]]/sum(breedingN15[[i]]),
                                             psi = results15[[i]]$psi )
...
compare15$MCA[1:nScenarios15 + 1] <- apply(compare15.array[,,'MCA'], 2, mean, na.rm = TRUE)
...
compare15 <- transform(compare15, MC.diff=MC - MC[1], MCA.diff=MCA - MCA[1],
                       ...
                       MC.prop=MC/MC[1], MCA.prop=MCA/MCA[1], 
                       Mantel.prop=Mantel/Mantel[1])


compare15a <- as.matrix(compare15[1 + 1:nScenarios15, c(2,5,4,7)])
...
#### EXAMPLE 10 (from process error simulation in Cohen et al. in revision)
...
# Crude optimizable function for developing migratory connectivity pattern based on MC
...
#### Simulation using error associated with light-level geolcators
```{r}
            ...
            # the input projection (projection.in) #
                   if(is.na(resolution)){
                     ...
                     resolution <- c(long.res,long.res)
                             ...
                             rast <- suppressWarnings(raster::rasterFromXYZ(cbind(cbind(siteCentroids),(1:nrow(siteCentroids))),
                             res = resolution,
                             crs = projection))
...
# convert breeding and wintering locations to ploygons using the helper function
