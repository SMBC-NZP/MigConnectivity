...
M<-estMC(isGL=OVENdata$isGL, # Logical vector indicating light-level geolocator (T) GPS (F)
         ...
         projection.dist.calc = raster::projection(OVENdata$targetPoints), #Lambert projection
...
winteringPolys2 <- lapply(winteringPos14,toPoly,projection.in = WGS84, projection.out = projection.dist.calc)
...
for (sim in 1:nSims14.2) {
  ...
  for (i in 13) {
    ...
    points1 <- sp::spTransform(points0, sp::CRS(projection.dist.calc))
    ...
    save(results14, est14.array, var14.array, ci14.array, file = '../results14.2.gzip')
  }
}
...
g.est <- ggplot(est14.2.df, aes(Estimate)) + geom_histogram(bins = 15) +
  ...
  theme_bw() + scale_x_continuous(breaks = c(0.002, 0.003, 0.1, 0.2, 0.3))
png('sim.est.MC.rM5.png', width = 6, height = 4, units = 'in', res=600)
g.est
#dev.off()
