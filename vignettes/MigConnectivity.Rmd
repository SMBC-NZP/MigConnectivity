...
geoBias <- OVENdata$geo.bias / 1000
geoVCov <- OVENdata$geo.vcov / 1000000
...
# the input projection (projection.in) #
  ...
  centers <- SpatialPoints(siteCentroids,CRS(projection.in))
...
targLocs14.2 <- lapply(targLocs14, sp::spTransform, CRSobj = CRS(Lambert))
...
system.time(for (sim in 1:nSimsLarge14.2) {
  ...
  sim14.2[[sim]] <- lapply(sampleBreeding14.2, simMove, breedingDist = breedDist14[[1]],
                  ...
                  nMonths=nMonths)
  for (i in 13) {
    animalLoc14[[i]] <- changeLocations(sim14.2[[sim]][[scenarioToSampleMap14[i]]]$animalLoc,
                                        breedingSiteTrans14[[i]], winteringSiteTrans14[[i]])
    winteringTruePoints14 <- targLocs14.2[[i]][animalLoc14[[i]][,2,1,1], ]
    results14.2 <- simLocationError(winteringTruePoints14, WinteringPolys[[i]], 
                                    geoBias, geoVCov, Lambert)
    animalLoc14[[i]][,2,1,1]==results14.2$targetSample
    ...
    compare14.2.array[sim, 'Mantel'] <- calcStrengthInd(breedDist14[[1]],
                                                      nonbreedDist14[[1]],
                                                      sim14[[sim]][[scenarioToSampleMap14[i]]]$animalLoc,
                                                      resamp=0)$correlation
  }
