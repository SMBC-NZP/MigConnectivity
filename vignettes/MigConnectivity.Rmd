...
1. `estMC` estimates the strength of migratory connectivity (MC) with estimates of uncertainty. MC estimated either with transition probability estimates or individual assignments to regions from tracking data. Uses resampling to measure MC uncertainty from RMark transition probability matrix estimates and/or JAGS relative abundance MCMC samples OR geolocator and/or GPS data.   
...
`mlogitMC`: Crude optimizable function for developing migratory connectivity pattern based on migratory connectivity strength.  Uses same argument (origin.abund) for relative or absolute abundance.

```{r}
mlogitMC <- function(slope, MC.in, origin.dist, target.dist, origin.abund) {
    nBreeding <- nrow(origin.dist)
    nWintering <- nrow(target.dist)
    psi <- mlogitMat(slope, origin.dist)
  if (any(psi<0))
    return(5*slope^2)
  if (sum(origin.abund) > 1)
    MC <- calcMC(origin.dist, target.dist, psi, originAbund = origin.abund)
  else
    MC <- calcMC(origin.dist, target.dist, psi, originRelAbund = origin.abund)
  return((MC.in - MC)^2)
}
```
...
```{r}
calcPsiMC <- function(originDist, targetDist, locations, originRelAbund = NULL, 
                      originAbund = NULL, verbose=F) {
  ...
  MC <- NULL #calcMC(originDist, targetDist, psiMat, originRelAbund, originAbund)
...
o15 <- optimize(mlogitMC, MC.in = 0.25, origin.dist = breedDist15[[1]],
                ...
                origin.abund = breedingN15[[1]], interval = c(0,10),
...
for (sim in 1:nSims15) {
  ...
  for (i in 1:nScenarios15) {
    ...
    results15[[i]] <- calcPsiMC(breedDist15[[i]], nonbreedDist15[[i]],
                                   animalLoc15[[i]], breedingRelN15[[i]], NULL, F)
    compare15.array[sim, i, 'MC'] <- results15[[i]]$MC
    compare15.array[sim, i, 'MCA'] <- calcMC(breedDist15[[i]], nonbreedDist15[[i]], 
                                             results15[[i]]$psi, 
                                             originAbund = breedingN15[[i]])
...
compare15$MCA[1:nScenarios15 + 1] <- apply(compare15.array[,,'MCA'], 2, mean)
...
for (sim in 1:nSims16) {
  ...
  for (i in 1:nScenarios16) {
    ...
    results16[[i]] <- calcPsiMC(breedDist16[[i]], nonbreedDist16[[i]],
                                animalLoc16[[i]], breedingRelN16[[i]])
    compare16.array[sim, i, 'MC'] <- results16[[i]]$MC
    compare16.array[sim, i, 'MCA'] <- calcMC(breedDist16[[i]], nonbreedDist16[[i]], 
                                             results16[[i]]$psi, 
                                             originAbund = breedingN16[[i]])
    compare16.array[sim, i, 'MCss1'] <- calcMC(breedDist16[[i]], nonbreedDist16[[i]], 
                                             results16[[i]]$psi, 
                                             originAbund = table(animalLoc16[[i]][ , 1, 1, 1]))
    compare16.array[sim, i, 'MCss2'] <- calcMC(breedDist16[[i]], nonbreedDist16[[i]], 
                                             results16[[i]]$psi, 
                                             originAbund = breedingRelN16[[i]] * nSample16) # pmax(breedingRelN16[[i]] * nSample16, 1))
...
compare16$MCA[1:nScenarios16 + 1] <- apply(compare16.array[,,'MCA'], 2, mean)
compare16$MCss1[1:nScenarios16 + 1] <- apply(compare16.array[,,'MCss1'], 2, mean)
compare16$MCss2[1:nScenarios16 + 1] <- apply(compare16.array[,,'MCss2'], 2, mean)
