...
author: "Jeff Hostetler, Mike Hallworth"
date: '`r Sys.Date()`'
output:
  rmarkdown::html_vignette: default
  rmarkdown::pdf_vignette: default
vignette: |
  %\VignetteIndexEntry{MigConnectivity package} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
---
```{r, echo = FALSE}
...
Cohen, E. B., J.A. Hostetler, M.T. Hallworth, C.S. Rushing, T.S. Sillett, P.P. Marra. In revision. A quantitative definition for the strength of migratory connectivity.
...
1. the relative or the absolute abundance within each of the regions where individuals originate from (here the breeding regions). 
...
relN
# Define total sample size for psi data for small sample corrected version of MC 
n <- 500
...
CapLocs<-sp::SpatialPoints(captureLocations,sp::CRS(WGS84))

# Project Capture locations # 

CapLocsM<-sp::spTransform(CapLocs, sp::CRS(Lambert))
...
NB_GL <- lapply(Non_breeding_files, FUN = function(x){sp::SpatialPoints(cbind(x$Longitude,x$Latitude),sp::CRS(WGS84))})

# Project into UTM projection #

NB_GLmeters <- lapply(NB_GL, FUN = function(x){sp::spTransform(x,sp::CRS(Lambert))})
...
The figure below shows the two breeding regions (squares), and the three non-breeding regions (gray scale) used in Cohen et al. (*in revision*) to estimate MC for Ovenbirds tracked with light-level geolocators and PinPoint-10 GPS tags.  
...
M<-estMC(isGL=OVENdata$isGL, # Logical vector indicating light-level geolocator (T) GPS (F)
         ...
         resampleProjection = raster::projection(OVENdata$targetPoints), #Lambert projection
...
simLocationError <- function(targetPoints, targetSites, geoBias, geoVCov, 
                             projection, verbose = 0, nSim = 1000) {
  ...
  for(i in 1:nAnimals){
    ...
    while (is.na(target.sample[i])) {
      ...
      point.sample <- sp::SpatialPoints(MASS::mvrnorm(n=nSim, mu=cbind(
        targetPoints@coords[i,1],
        targetPoints@coords[i,2]), Sigma=geoVCov)+
          geoBias2, sp::CRS(projection))
  ...
  return(list(targetSample = target.sample, targetPointSample = target.point.sample))
}
```
...
# the input projection (projection.in) #
  ...
  rast <- suppressWarnings(raster::rasterFromXYZ(cbind(cbind(siteCentroids),
...
Cohen et al. (*in revision*) used simulation to assess the influence of the three data inputs needed to calculate MC, including:  
...
#### Calculate MC for breeding linear / winter linear arrangment 
```{r}
...
#### Calculate MC for breeding grid/ winter grid arrangement
```{r}
...
#### Calculate MC for breeding grid, winter linear arrangement
```{r}
...
### EXAMPLES 4-11 (from sampling regime simulations in Cohen et al. in revision)
...
compare15$MC[1:nScenarios15 + 1] <- apply(compare15.array[,,'MC'], 2, mean, na.rm = TRUE)
compare15$Mantel[1:nScenarios15 + 1] <- apply(compare15.array[,,'Mantel'], 2,mean, na.rm = TRUE)
...
compare16$Mantel[1:nScenarios16 + 1] <- apply(compare16.array[,,'Mantel'], 2, mean, na.rm = TRUE)
...
#### EXAMPLE 7
`estMC` Estimate strength of migratory connectivity incorporating uncertainty from low sample size (no other sampling error).  Here we simulated data using the 13th scenario from example 4 above ("CentroidSampleBreeding4"; make sure you run example 4 code before running this section).  

```{r, eval = FALSE}
WGS84 <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
  
set.seed(75)
capLocs14 <- lapply(breedingPos14, sp::SpatialPoints, proj4string = sp::CRS(WGS84))
targLocs14 <- lapply(winteringPos14, sp::SpatialPoints, proj4string = sp::CRS(WGS84))
...
compare14.1.array <- array(NA, c(nSimsLarge14, 2), dimnames =
                           list(1:nSimsLarge14,
                                c("MC", "Mantel")))
...
system.time(for (sim in 1:nSimsLarge14) {
  ...
  sim14[[sim]] <- lapply(sampleBreeding14.1, simMove, breedingDist = breedDist14[[1]],
                  winteringDist=nonbreedDist14[[1]], psi=psi, nYears=nYears,
                  nMonths=nMonths)
  for (i in 13) {
...
summary(est14.array)
means14
summary(ci14.array[, "MC", 'lower'] <= MC14 & ci14.array[, "MC", 'upper'] >= MC14)
summary(ci14.array[, "Mantel", 'lower'] <= MC14 & ci14.array[, "Mantel", 'upper'] >= MC14)
...
png('sim.est.MC.rM4.png', width = 6, height = 4, units = 'in', res=600)
g.est
#dev.off()
...
#### EXAMPLE 8
`estMC` Estimate strength of migratory connectivity incorporating uncertainty from location error associated with light-level geolocators.  Again, we simulated data using the 13th scenario from example 4 above ("CentroidSampleBreeding4"; make sure you run example 4 code before running this section).  
...
# convert breeding and wintering locations to polygons using the helper function
BreedingPolys <- lapply(breedingPos14, toPoly, projection.in = WGS84, projection.out = Lambert)
breedingPolys <- lapply(breedingPos14, toPoly, projection.in = WGS84, projection.out = WGS84)

WinteringPolys <- lapply(winteringPos14,toPoly,projection.in = WGS84, projection.out = Lambert)
winteringPolys <- lapply(winteringPos14,toPoly,projection.in = WGS84, projection.out = WGS84)
winteringPolys2 <- lapply(winteringPos14,toPoly,projection.in = WGS84, projection.out = resampleProjection)
targLocs14.2 <- lapply(targLocs14, sp::spTransform, CRSobj = sp::CRS(Lambert))


# Double check that the input center points and output polygons are ordered correctly 
cbind(breedingPos14[[1]][,1:2],t(sapply(slot(breedingPolys[[1]], "polygons"), function(x){ slot(x,"labpt")})))
cbind(winteringPos14[[1]][,1:2],t(sapply(slot(winteringPolys[[1]], "polygons"), function(x){ slot(x,"labpt")})))
  

# simulateLocs <- function(n,n.origin = FALSE,originSites,targetSites,projection){
#                 if(n.origin){
#                 n.polys <- length(originSites)
#                 originPoints <- vector('list',n.polys)
#                 targetPoints <- vector('list',n.polys)
#                 
#                 for(i in 1:n.polys){
#                   originPoints[[i]]<-sp::spsample(originSites[i,],n,type = "random")@coords
#                   targetPoints[[i]]<-sp::spsample(targetSites,n,type = "random")@coords
#                 }
#                 
#                 originPts <- do.call('rbind',originPoints)
#                 targetPts <- do.call('rbind',targetPoints)
#                 
#                 return(list(originPoints = sp::SpatialPoints(originPts,proj4string = sp::CRS(projection)),
#                             targetPoints = sp::SpatialPoints(targetPts,proj4string = sp::CRS(projection))))
#                 
#                 }
#                else{
#                 originPoints <- sp::spsample(originSites,n,type = "random")
#                 targetPoints <- sp::spsample(targetSites,n,type = "random")
#                 
#                 return(list(originPoints = originPoints,
...
sampleBreeding14.2 <- list(round(breedingRelN14[[1]]*nSample14.2),
                         c(rep(0, 22), round(breedingRelN14[[5]][1]*nSample14.2),
                           ...
                           rep(0, 22)),
                         rep(c(rep(0, 4),
                               rep(round(breedingRelN14[[2]][1]*nSample14.2/2), 2),
                               rep(0, 4)), 10))
...
# Set up data structures for storing results
animalLoc14 <- vector("list", nScenarios14) #making an empty list to fill
sim14.2 <- vector("list", nSimsLarge14.2) #making an empty list to fill

compare14.2.array <- array(NA, c(nSimsLarge14.2, 2), dimnames =
                           list(1:nSimsLarge14.2,
                                c("MC", "Mantel")))
...
system.time(for (sim in 1:nSimsLarge14.2) {
  ...
  sim14.2[[sim]] <- lapply(sampleBreeding14.2, simMove, breedingDist = breedDist14[[1]],
                  winteringDist=nonbreedDist14[[1]], psi=psi, nYears=nYears,
                  nMonths=nMonths)
  for (i in 13) {
    ...
    results14.2[[sim]] <- simLocationError(winteringTruePoints14, WinteringPolys[[i]], 
                                    geoBias, geoVCov, Lambert)
#    summary(animalLoc14[[i]][,2,1,1]==results14.2[[sim]]$targetSample)
#    results14.2$targetPointSample - winteringTruePoints14@coords
    ...
    originDist0 <- geosphere::distVincentyEllipsoid(breedingTruePoints14[distIndices[,'row']],
                                         breedingTruePoints14[distIndices[,'col']])
    originDist1[lower.tri(originDist1)] <- originDist0
    target.point.sample <- sp::SpatialPoints(results14.2[[sim]]$targetPointSample,sp::CRS(Lambert))
    ...
    compare14.2.array[sim, 'Mantel'] <- ncf::mantel.test(originDist1, targetDist1, 
                                                         resamp=0, quiet = T)$correlation
  }
})
...
# Set up data structures for storing estimation results
est14.2.array <- array(NA, c(nSims14.2, 2), dimnames =
                           list(1:nSims14.2,
                                c("MC", "Mantel")))
var14.2.array <- array(NA, c(nSims14.2, 2), dimnames =
                           list(1:nSims14.2,
                                c("MC", "Mantel")))
ci14.2.array <- array(NA, c(nSims14.2, 2, 2), dimnames =
                           list(1:nSims14.2,
                                c("MC", "Mantel"),
                                c('lower', 'upper')))
...
for (sim in 1:nSims14.2) {
  ...
  for (i in 13) {
    points0 <- sp::SpatialPoints(results14.2[[sim]]$targetPointSample, sp::CRS(Lambert))
    points1 <- sp::spTransform(points0, sp::CRS(resampleProjection))
    animalLoc14[[sim]] <- changeLocations(sim14.2.sub[[sim]][[scenarioToSampleMap14[i]]]$animalLoc,
                                        breedingSiteTrans14[[i]], winteringSiteTrans14[[i]])
    results14[[sim]] <- estMC(originDist = breedDist14[[i]], targetDist = nonbreedDist14[[i]],
                            originRelAbund = breedingRelN14[[i]],
                            originPoints = capLocs14[[i]][animalLoc14[[sim]][, 1, 1, 1], ],
                            ...
                            nSamples = 1000, verbose = 0, calcCorr = T,
                            geoBias = geoBias, geoVCov = geoVCov, isGL = T,
    ...
    save(results14, est14.2.array, var14.2.array, ci14.2.array, file = '../results14.2.gzip')
  }
}
...
means14.2
summary(ci14.2.array[, "MC", 'lower'] <= MC14 & ci14.2.array[, "MC", 'upper'] >= MC14)
summary(ci14.2.array[, "Mantel", 'lower'] <= MC14 & ci14.2.array[, "Mantel", 'upper'] >= MC14)

est14.2.df <- data.frame(Parameter = rep(c("MC", 'rM'), 2, each = nSims14.2),
                     Quantity = rep(c('Mean', 'Variance'), each = 2 * nSims14.2),
                     sim = rep(1:nSims14.2, 4), Estimate = c(est14.2.array, var14.2.array))
...
g.est <- ggplot(est14.2.df, aes(Estimate)) + geom_histogram(bins = 15) +
  ...
  theme_bw() + scale_x_continuous(breaks = c(0.002, 0.003, 0.1, 0.2, 0.3))
png('sim.est.MC.rM6.png', width = 6, height = 4, units = 'in', res=600)
g.est
#dev.off()

qualities14.2 <- data.frame(Parameter = rep(c("MC", 'rM'), 8),
                       Quantity = rep(rep(c('Mean', 'Variance'), c(4, 2)), 3, 16),
                       Measure = rep(c('Bias', 'RMSE', "Coverage"), c(6, 6, 4)),
                       Value = c(colMeans(est14.2.array - MC14),
                                 colMeans(var14.2.array) - vars14.2,
                                 sqrt(colMeans((est14.2.array - MC14)^2)),
                                 sqrt(mean((var14.2.array[,1] - vars14[1])^2)),
                                 sqrt(mean((var14.2.array[,2] - vars14[2])^2)),
...
# nSimInds <- 10
# 
# simLocs <- simulateLocs(n = nSimInds,n.origin = TRUE,originSites = BreedingPolys[[14]], targetSites = WinteringPolys[[14]],projection = Lambert)

# simulate relative abundance within each originSite 
# simRelAbun <- rep(1/length(BreedingPolys[[14]]),length(BreedingPolys[[14]]))
# 
# sim.isGL <- rep(TRUE,length(simLocs$originPoints))
# 
# plot(union(BreedingPolys[[14]],WinteringPolys[[14]]))
# plot(simLocs$originPoints,pch = ".", col = "red", add = TRUE)
# plot(simLocs$targetPoints,pch = ".", col = "blue", add = TRUE)
# 
# OVENdata$originDist
# 
# originDist <- as.matrix(dist(getSpPPolygonsLabptSlots(BreedingPolys[[14]])))
# 
# targetDist <- as.matrix(dist(getSpPPolygonsLabptSlots(WinteringPolys[[14]])))


#simGL<-estMC(isGL=sim.isGL, # Logical vector indicating light-level geolocator (T) GPS (F)
#             geoBias = geoBias, # Light-level geolocator location bias
#             geoVCov = geoVCov, # Light-level geolocator co-variance matrix
#             targetDist = targetDist, # Non-breeding target distribution distance matrix
#             originDist = originDist, # Breeding / origin distribution distance matrix
#             targetSites = WinteringPolys[[14]], # Non-breeding target sites
#             originSites = BreedingPolys[[14]], # Breeding origin sites 
#             originPoints = simLocs$originPoints, # Capture Locations 
#             targetPoints = simLocs$targetPoints, # Non-breeding Locations derived from devices
#             originRelAbund = simRelAbun, # Relative abundance within OriginSites
#             verbose = 0,   # output options - see help ??estMC
#             nSim = 10,     # This is set low for example
#             nSamples = 10) # This is set low for example 
```
...
#### EXAMPLE 11
`estMC` Estimate strength of migratory connectivity incorporating location uncertainty (***Examples*** see above)
...
#### EXAMPLE 12 (from process error simulation in Cohen et al. in revision)
...
for(i in 1:length(Drates)){
  ...
  for(j in 1:nYears){
    ...
    temp.results <- calcPsiMC(breedDist, nonbreedDist, breedingRelN,
                                 birdLocDisp[[i]]$animalLoc, years=j, verbose = F)
