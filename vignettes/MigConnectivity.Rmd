...
#### EXAMPLE 7
`estMC` Estimate strength of migratory connectivity incorporating uncertainty from low sample size (no other sampling error).  Here we simulated data using the 13th scenario from example 4 above ("CentroidSampleBreeding4"; make sure you run example 4 code before running this section).  We changed the total sample size from 1000 to 100 animals.  
...
qualities14 <- data.frame(Parameter = rep(c("MC", 'rM'), 5),
                       Quantity = rep(rep(c('Mean', 'Variance'), c(2, 2)), 3, 10),
                       Measure = rep(c('Bias', 'RMSE', "Coverage"), c(4, 4, 2)),
...
for (sim in 1:nSims14.2) {
  ...
  for (i in 13) {
    ...
    save(results14, est14.2.array, var14.2.array, ci14.2.array, file = '../results14.2b.gzip')
  }
}
...
means14.2
mean(ci14.2.array[, "MC", 'lower'] <= MC14 & ci14.2.array[, "MC", 'upper'] >= MC14)
mean(ci14.2.array[, "Mantel", 'lower'] <= MC14 & ci14.2.array[, "Mantel", 'upper'] >= MC14)
sqrt(vars14.2) / MC14
...
save.image('../estMCsims.14.2.RData')

load('../estMCsims.c.RData')
load('../estMCsims.14.2.RData')
...
g.est <- ggplot(est14.all.df, aes(Estimate)) + geom_histogram(bins = 12) +
  ...
  theme_bw() + scale_x_continuous(breaks = c(0.002, 0.003, 0.1, 0.2, 0.3))
png('../sim.est.MC.rM7.png', width = 6, height = 8, units = 'in', res=600)
g.est
dev.off()
