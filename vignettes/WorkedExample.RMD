...
author: "Jeffrey A. Hostetler, Michael T. Hallworth, Henry Stevens"
date: '`r Sys.Date()`'
output:
...
Below we provide a worked example using Yellow Warbler (<i>Setophaga petechia</i>) data [@hostetler_challenges_2025] to help users format their data to use the new 
...
sp_path <- ebirdst_download_status(species = "yelwar", path = "./data/")

ebird <- load_raster("yelwar", path = "./data/",
                     product = "abundance", 
                     period = "seasonal",
                     metric = "mean",
                     resolution = "9km")  #medium resolution to match old code

## NOTE: parse_raster_dates is now defunct.
# workaround -- divide abundance raster by season and estimate relative abundance

b <- ebird[['breeding']]
w <- ebird[['nonbreeding']]

targetSitesProj <- sf::st_transform(targetSitesYEWA, terra::crs(w))
originSitesProj <- sf::st_transform(YEWA_origin_sites, terra::crs(b))

relAbundYEWAWint <- terra::extract(w, targetSitesProj, fun = 'sum', na.rm = T)
relAbundYEWAWint$rel <- relAbundYEWAWint$nonbreeding / 
  sum(relAbundYEWAWint$nonbreeding)
relAbundYEWAWint <- relAbundYEWAWint$rel
names(relAbundYEWAWint) <- targetNames

relAbundYEWABreed <- terra::extract(b, originSitesProj, fun = 'sum', na.rm = T)
relAbundYEWABreed$rel <- relAbundYEWABreed$breeding / 
  sum(relAbundYEWABreed$breeding)
relAbundYEWABreed <- relAbundYEWABreed$rel
names(relAbundYEWABreed) <- originNames

```
```{r, echo = FALSE}
relAbundYEWABreed <- c(Arctic = 0.1644263, 'Pacific Northwest' = 0.1167522,  
                       Southwest = 0.2902441, Central = 0.2394759,
                       East = 0.1891015)
relAbundYEWAWint <- c('Pacific and Central Mexico' = 0.09903443,    
                      'Atlantic Lowland Mexico' = 0.18604479,
                      'Central America' = 0.43239219, 
                      'South America' = 0.28252859)
```
```{r}
relAbundYEWAWint
relAbundYEWABreed
