...
library(methods)
library(VGAM)
instance <- 211 # as.integer(args[1])
set.seed(instance)
...
sampleSize[[2]] <- list(300 * originRelAbund, NULL)
...
sampleSize[[5]] <- list(rep(100, nOriginSites), NULL)
...
sampleSizeGeno <- lapply(sampleSize, function(x) x[[1]])
...
shapes <- matrix(c(7.5, 0.2, 0.2, 0.01, 0.01,
                   0.2, 7.5, 0.2, 0.01, 0.01,
                   0.2, 0.2, 7.5, 0.01, 0.01,
                   0.005, 0.005, 0.005, 5.5, 0.25,
                   0.005, 0.005, 0.005, 0.25, 5.5),
                 nTargetSites, nTargetSites, byrow = T,
...
for (sim in 1:nSims) {
  ...
  for (sc in fastScenarios) { #1:nScenarios
    ...
    est1 <- estTransition(originSites,
                          targetSites,
                          targetAssignment = tt,
                          originAssignment = oa, maxTries = 1000,
                          #originRaster = or, #
