...
library(methods)
library(VGAM)
instance <- 209# as.integer(args[1])
set.seed(instance)
...
# Modified to change lowest psi>=0.01 to roughly half its value, and spread the
# other half equally among psis from same row that were <0.01
psiTrue <- array(NA, c(nOriginSites, nTargetSites))
...
psiTrue[1,] <- c(0.05304, 0.10856, 0.78536, 0.02652, 0.02652)
psiTrue[2,] <- c(0.02221, 0.48664, 0.46895, 0.01110, 0.01110)
psiTrue[3,] <- c(0.07594, 0.07594, 0.07594, 0.54435, 0.22783)
rowSums(psiTrue)
...
shapes <- matrix(c(5.5, 0.1, 0.1,
                   0.1, 5.5, 0.1,
                   0.1, 0.1, 5.5), 3, 3, byrow = T,
